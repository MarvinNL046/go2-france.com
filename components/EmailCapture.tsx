...
export default function EmailCapture({
  ...
  if (variant === 'sidebar') {
    return (
      <div className={`bg-surface-dark text-white rounded-2xl p-6 ${className}`}>
        <span className="section-label font-script text-brand-accent text-sm">Stay in the loop</span>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            ...
            className="w-full bg-france-red text-white font-medium py-2 rounded-xl hover:bg-france-red/90 transition-colors disabled:opacity-50"
          >
      ...
      <div className={`${className}`}>
        ...
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input
            ...
            required
            className="flex-1 px-4 py-3 rounded-xl border border-gray-200 text-gray-900 text-sm focus:outline-none focus:border-france-blue focus:ring-1 focus:ring-france-blue transition-colors"
          />
          <button
            type="submit"
            disabled={status === 'loading'}
            className="bg-france-blue text-white font-semibold px-6 py-3 rounded-xl hover:bg-blue-800 transition-colors disabled:opacity-50 whitespace-nowrap"
          >
            {status === 'loading' ? '...' : b}
          </button>
        </form>
  ...
  return (
    <div className={`bg-gradient-to-r from-france-blue/5 to-france-red/5 border border-france-blue/15 rounded-2xl p-6 my-8 ${className}`}>
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <form onSubmit={handleSubmit} className="flex gap-2 sm:min-w-[320px]">
          <input
            ...
            placeholder="your@email.com"
            required
            className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-gray-900 text-sm focus:outline-none focus:border-france-blue focus:ring-1 focus:ring-france-blue transition-colors"
          />
          <button
            type="submit"
            disabled={status === 'loading'}
            className="bg-france-blue text-white font-semibold px-5 py-2.5 rounded-xl hover:bg-blue-800 transition-colors disabled:opacity-50 whitespace-nowrap text-sm"
          >
            {status === 'loading' ? '...' : b}
          </button>
        </form>
